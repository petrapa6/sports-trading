...
- Not ticked yet: `npm run feeds:smoke` against the real NHL API. The development session's egress policy blocks `api-web.nhle.com` (the request is answered `403` by the proxy), so the script was run there only against the fixture stand-in (`NHL_SCRIPT_BASE_URL`, output format checked by `verify:T07`). CI has a `feeds-smoke` job that runs it against the real API on every push; the item is ticked once that job is green (run URL in `docs/verification/T07.md`).
