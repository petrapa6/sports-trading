...
- T11 (importers, candle collector, price-model builder) was not implemented before T12. The simulator reads `hist_games`, `markets` and `hist_prices` directly, so it works on whatever those tables hold (today: timelines the tracker archives, `source='live'`); the tests and the e2e spec seed the tables themselves. T12 defines the stored price-model format T11 must write to `settings.price_model`: `{builtAt?, cells: [{sport, lead: 1|2|3, remaining: 0|5|10|…, askBp, sampleSize}]}` (`src/backtest/priceModel.ts`, which also exports the seed table for T11's "empty DB → full seed table").
- Seed table: anchored on the §9 values (soccer lead 2 / 10 min left $0.96, lead 1 / 10 min $0.85, hockey lead 2 / 5 min $0.97) and moving per 5-minute bucket by 3¢ (soccer lead 1), 1¢ (soccer lead 2), 4¢ (hockey lead 1), 1.5¢ (hockey lead 2); lead 3+ = lead 2 + 2.5¢ (soccer) / 1.5¢ (hockey); clamped to $0.50–$0.99. A cell with < 20 observations uses the seed and reports its own sample size; `minSampleSize` is the smallest sample size behind any price the run used (0 = seed only).
- Game minute → candle minute (`src/backtest/clock.ts` `candleMinuteMs`): soccer kick-off + minute for the first half; second half from the observed second-half start (`games.second_half_observed_at` of live-archived games) + (minute − 45), else kick-off + minute + 17 (stoppage + break); hockey kick-off + 55 min per completed period (37 of play + 18 intermission) + elapsed period minutes × 37 / 20. Kick-off is `games.kickoff_observed_at` when known, else `hist_games.played_at`. Exact prices are therefore as good as this mapping; T11 may store a better clock.
- Ticks: a goal recorded at minute `m` is in the score from tick `m`; soccer ticks 1–90, hockey 1–60 with period / seconds-left derived so `ruleMinute()` equals the live NHL minute. The simulator never sees halftime, blocked feeds, stale feeds, closed markets or exchange pauses (those guards are omitted); depth is not modelled (one ask level of unlimited size, `minDepthContracts` ignored), so a fill is always the full requested size; fee multiplier 1 (every configured series).
