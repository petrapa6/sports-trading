...
- [x] Order body: ask 0.93, `maxSlippage` 0.01, `maxPrice` 0.97, stake 2 000 000 → body `{"ticker":…, "side":"bid", "count":"2", "price":"0.9400", "time_in_force":"immediate_or_cancel", "self_trade_prevention_type":"taker_at_cross", "client_order_id":"<trade.id>-1", "order_group_id":"<id>"}`; with `KALSHI_SUBACCOUNT=3` also `"subaccount":3`; the `pending` attempt row exists before the request.
- [x] Response `fill_count:"2.00", average_fill_price:"0.9300", average_fee_paid:"0.0046"` → trade `filled`, `fill_cc 200`, `avg_fill_price_bp 9300`, `cost_micros 1860000`, `fee_micros 9200`, `effective_mode 'live'`; `fill_count:"1.00"` → `filled` with 100 cc; `"0.00"` → attempt `unfilled`, trade `waiting`, retried next tick; HTTP 400/409 → `skipped/order_rejected` with the error message stored; order-group rejection → `waiting/order_group_limit`.
- [x] Live sizing: balance $100 with one `pending` live attempt costing $3 → sizing base 97 000 000 micros.
- [x] Recovery: a `pending` live attempt + `getOrders` returning an executed order with our `client_order_id`, `fill_count_fp "2.00"`, `taker_fill_cost_dollars "1.8600"`, `taker_fees_dollars "0.0092"` → trade `filled` with those values; an empty list (and empty historical list) → `unfilled/restart_no_order`; recovery completes before the first scheduler tick (order asserted); the request never contains `client_order_id` as a query parameter.
- [x] Reconciliation: settlements reporting revenue 20 000 micros different → `reconcile_warning` set and one `warn`; equal → none.
- [x] `balance_snapshots`: +15 min (fake timers) → one row; after a mocked fill → an extra row within 1 s; global kill switch on → no rows and no requests.
- [x] Switch matrix with a live-configured strategy: `allow_live_orders=false` → zero requests to `/portfolio/events/orders` (msw spy), trade `dry_run/addon_lock`; global dry run on → zero order requests, `dry_run/global_dry_run`; strategy kill switch turned on mid-window → `skipped/paused`; global kill switch on → zero requests of any kind. `maxStakeUsd` 1 with ask 0.93 → `count:"1"`.
- [x] Step-up: switching a strategy to live without recent re-auth → `403`; with re-auth → `200` and an audit row; the e2e confirms the prompt.
- [x] Client allow-list test passes and fails if a method named `withdraw`, `deposit` or `transfer` is added (mutation test in the verification doc).
...
- [x] `npm run replay -- --live-mock` with a live strategy (`allow_live_orders=true`, global dry run off) against the msw Kalshi mock → one live trade that fills and settles; `balance_snapshots` grows; `/api/stats` shows it only under `live`.

**Implementation notes (T13, deviations and clarifications)**
- Live attempt order: the attempt row is inserted (`pending`) first, then market, exchange status, orderbook and `GET /portfolio/balance` are read and the §5 guards run with the sizing base `cash − Σ(requested_cc × limit_price_bp)` of the app's other `pending` live attempts. The count sent is `floor(min(sizing contracts, contracts offered ≤ limit))` whole contracts (0 → soft `liquidity`). The switches are read once more right before the order (a kill switch → `hard_skip/paused`; a strategy no longer effectively live → soft `mode_changed`, retried next tick in its new mode). Then, in one transaction, the attempt row gets its limit and count and the trade goes `pending` (with balance, stake, limit, requested); only then is Create Order V2 sent. A trade therefore goes `signalled/waiting → pending → filled | waiting | skipped` in live, while dry run still writes `pending → filled` in one transaction.
- Outcomes: `fill_count > 0` → attempt and trade `filled` with the exchange's count, `average_fill_price`, `cost = fill_cc × avg_bp` and fee `average_fee_paid × fill_count`; `0` → attempt `unfilled/unfilled`, trade `waiting` (retried on the next matching tick). A 4xx other than 429 → `hard_skip/order_rejected` with `{error, status, code}` stored in `trade_attempts.response`; a rejection whose error code or message names the order group (Kalshi's exact code is not documented; `/order[ _-]?group/i`, 404 excluded) → soft `order_group_limit`, one `warn`, and Settings → Trading shows the group as "limit hit" until it is reset; a 404 naming the order group marks the stored group unknown so the next live order creates a new one (attempt `error`). When the outcome is unknown — no HTTP answer, 5xx/429 after backoff, an unreadable 201 — the order is looked up by `client_order_id` at once (as in recovery); found → applied, not found → attempt `error` (soft); lookup failing → the attempt stays `pending` and every settler run (60 s) resolves pending live attempts older than 30 s (`order_not_found` when Kalshi has no such order). `NetworkPaused` from the gate means the order was never sent → `error`.
- Recovery (`Executor.recoverOnStart`, now async; `core/startup.ts` runs order group → recovery → scheduler, settler, balance recorder, in that order): pending live attempts are matched by `client_order_id` among `getOrders({ticker, minTs: attempt.at − 60 s})`, then `getHistoricalOrders` with the same filter; a match applies `fill_count_fp`, `taker_fill_cost_dollars` (cost), `taker_fees_dollars` (fee) and `avg_fill_price_bp = round(cost / fill_cc)`; a match with nothing filled → `unfilled/unfilled`; no match → `unfilled/restart_no_order`. The trade then goes `filled`, or `waiting` / `skipped` (`window_expired = 1`) by `window_ends_at`. A lookup that fails (network, kill switch on) leaves the attempt `pending` for the settler loop. Dry-run recovery is unchanged.
- Order group (`core/orderGroup.ts`): ensured at start-up only when `allow_live_orders` is on and Kalshi is configured — the stored `kalshi_order_group_id` is reused while `GET /portfolio/order_groups/{id}` answers (a network error keeps it), a 404 creates a new one with `order_group_contract_limit` (audited `order_group_created`, `mode = 'live'`). `GET /api/settings/order-group` returns `{enabled, id, contractsLimit, state, lastError, checkedAt, limitHitAt}`; `POST /api/settings/order-group/reset` (step-up, `409 live_orders_disabled` while the lock is closed) calls the exchange reset (or creates a group when the exchange no longer knows it) and is audited `order_group_reset`. The contract limit is editable in Settings → Trading and applies to the next group created.
- Settler live path: live trades of a settled market wait until `GET /portfolio/settlements?ticker=` lists the ticker (at most 30 min after the market was first seen settled; then they settle with the warning "Kalshi listed no settlement …"). The exchange's `revenue` is compared with the sum of the payouts of the app's live trades on that ticker; a difference above 10 000 micros sets the same `reconcile_warning` text on each of them and logs one `warn`. The Trades page shows "⚠ reconcile" in the row and the text in the expanded detail.
- `balance_snapshots` (`core/balances.ts`): one row every 15 minutes (not at start-up) and 250 ms after each live fill or live settlement (bursts merged); nothing — no row, no request — while the global kill switch is on. Written whenever Kalshi is configured (also while trading in dry run), for the environment and subaccount the app runs against.
- New attempt reasons beyond the §5 table: `restart_no_order`, `order_not_found`, `mode_changed`; `live_not_implemented` is gone (listed in the Trades status filter with the live reasons).
- Allow-list: `test/unit/kalshi/allowlist.test.ts` lists every method of `KalshiClient` (including the TypeScript-private plumbing on the prototype); a new method fails it until listed, and a name matching `/withdraw|deposit|transfer/i` always fails it. The client gained one read, `listMarkets` (used by `e2e:demo`).
- `npm run e2e:demo` (`scripts/e2e-demo.ts`): demo only (like `kalshi:smoke`, it prints `SKIPPED (no demo key)` without a key or with `kalshiEnv` other than `demo`); it picks the cheapest open market of the app's enabled series (else any open market) that offers ≥ 1 contract at its best ask, orders 1 contract at that ask, files the trade under a strategy "Demo fee check (npm run e2e:demo)" (kill switch on, mode live) with the same row/attempt/audit invariants as the executor, reads the order back and compares the fee with `feeMicros` at 100 and 10 000.
- `npm run replay -- --live-mock` runs in-process (temporary database, `scripts/live-mock-lib.ts`) instead of posting to a running server: the msw stand-in fills IOC orders at the best ask and keeps a balance, so no dev server or Kalshi account is involved.
- Not verified in the development session: `npm run e2e:demo` with a real demo key (no key available) — so the fee precision against real fills is still open and `fee_balance_precision_micros` stays at its default 100; the fixture stand-in (whose recorded order carries `average_fee_paid 0.0046` for 2 contracts at 0.93) matches precision 100. `docker compose up --build` was not run (no Docker daemon); the `Image` workflow covers it.
- Acceptance item "`npm run e2e:demo` with a demo key" stays unticked: only its no-key half (`SKIPPED (no demo key)`) and a run against the fixture stand-in were verified; the Trades page's `LIVE` badge with environment `demo` for a live trade is covered by `test/e2e/live.spec.ts` with a seeded row.
